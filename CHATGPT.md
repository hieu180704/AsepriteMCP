# AsepriteMCP — ChatGPT / Codex

> 💡 Gõ `/init` để AI phỏng vấn bối cảnh và hoàn tất thiết lập tài liệu dự án.

**Cách nạp ngữ cảnh**

- **Codex CLI** đọc `AGENTS.md` ở gốc dự án — nội dung ràng buộc giống hệt file này. MCP server khai
  báo trong `~/.codex/config.toml`; snippet TOML sẵn sàng copy nằm ở `.codex/config.snippet.toml`,
  sinh bằng `node scripts/setup-mcp.js`.
- **ChatGPT web** không đọc được file trong repo. Dán nội dung `.openai/system-prompt.txt` vào ô
  Instructions của Custom GPT — file đó đã chứa đủ ràng buộc, không cần truy cập repo.
- Rules đầy đủ ở `.agents/rules/`, recipes ở `.agents/recipes/`, skills ở `.agents/skills/`.
  Người dùng gõ `/asset-qc` thì đọc `.agents/skills/asset-qc/SKILL.md` rồi làm đúng quy trình đó.
